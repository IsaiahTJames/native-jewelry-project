...
- ssh into the server
    - For Mac and Linux: `ssh root@loveadornedtest.org`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo ijames2`
    - replace ijames2 with your own
- create password
    - `passwd ijames2`
- Move .ssh key file
    - `cp -r .ssh /home/ijames2`
- change ownership of the .ssh directory
    - `cd /home/ijames2`
    - `chown -R ijames2:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh ijames2@loveadornedtest.org`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- reboot
    - `sudo reboot`
- ssh back in
- `ssh ijames2@loveadornedtest.org`
...
- create a directory for your deployment
    - `sudo mkdir /var/www/loveadornedtest.org/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/loveadornedtest.org/html`
...
- `sudo vim /etc/nginx/sites-available/loveadornedtest.org`
...
server {
    ...
    root /var/www/loveadornedtest.org/html;
    index index.html index.htm index.nginx-debian.html;

    server_name loveadornedtest.org www.loveadornedtest.org;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/loveadornedtest.org /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/ijames2/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/ijames2/.ssh/github_deploy_key.pub >> /home/ijames2/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/ijames2/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H loveadornedtest.org >> ~/.ssh/known_hosts 
         scp -r ./dist/* gkephart@loveadornedtest.org:/var/www/loveadornedtest.org/html
```
